...
public class MainGame extends AdScene {
	...
	protected void initLevel() {
		...
		AdPrefs.resetAccessCount(AdEnviroment.getInstance().getContext(),
				"enemy");
		AdPrefs.resetAccessCount(AdEnviroment.getInstance().getContext(),
				"enemy_killed");
		...
		if (AdPrefs.getValue(AdEnviroment.getInstance().getContext(), "level",
				1) < GameData.getInstance().mMyLevel.getScore() - 1)
			AdPrefs.setValue(AdEnviroment.getInstance().getContext(), "level",
					GameData.getInstance().mMyLevel.getScore() - 1);
	...
	public void checkLevelFinish() {
		...
		if (this.mGameOver == false
				&& this.mLevelFinish == false
				&& SimplePreferences.getAccessCount(AdEnviroment.getInstance()
						.getContext(), "enemy_killed") >= 22 + 4 * diff) {
			...
			registerUpdateHandler(new TimerHandler(6, false,
					new ITimerCallback() {
						@Override
						public void onTimePassed(TimerHandler pTimerHandler) {
	...
	public void gameOver() {
		if (this.mGameOver == false && this.mLevelFinish == false) {
			...
			try {
				 ...
				 * Leaderboard l = new
				 * Leaderboard(AdEnviroment.getInstance().getContext
				 * ().getString(R.string.score)); s.submitTo(l, new
				 * Score.SubmitToCB() {
				 * 
				 * @Override public void onSuccess(boolean newHighScore) {
				 * ((Activity)
				 * AdEnviroment.getInstance().getContext()).setResult
				 * (Activity.RESULT_OK); }
				 * 
			...
			try {
				 ...
				 * Leaderboard l = new
				 * Leaderboard(AdEnviroment.getInstance().getContext
				 * ().getString(R.string.level)); s.submitTo(l, new
				 * Score.SubmitToCB() {
				 * 
				 * @Override public void onSuccess(boolean newHighScore) {
				 * ((Activity)
	...
	public void clearScene() {
		...
		for (int i = 0; i < getChild(GAME_LAYER).getChildCount(); i++) {
			...
			if (elem.getChildCount() > 0
					&& elem.getFirstChild() instanceof Plant)
	...
	public void manageAreaTouch(ITouchArea pTouchArea) {
		if (pTouchArea instanceof Card) {
			...
			if (field.getChildCount() == 1
					&& !(field.getFirstChild() instanceof Plant)) {
				GameData.getInstance().mSoundSeed.play();
				GameData.getInstance().mMySeed.addScore(1);
				field.getFirstChild().detachSelf();
			} else {
				if (this.mSelect != null && this.mSelect.isReady()
						&& field.getChildCount() == 0) {
					if (GameData.getInstance().mMySeed.getScore() >= this.mSelect
							.getPrice()) {
						GameData.getInstance().mMySeed.addScore(-this.mSelect
								.getPrice());
	...
	private void firstRushEnemy() {
		...
		registerUpdateHandler(new TimerHandler(12f, false,
				new ITimerCallback() {
					@Override
					public void onTimePassed(TimerHandler pTimerHandler) {
	...
	private void secondRushEnemy(final int pNum) {
		...
		for (int i = 0; i < n; i++) {
			...
			registerUpdateHandler(new TimerHandler(delay, false,
					new ITimerCallback() {
						@Override
						public void onTimePassed(TimerHandler pTimerHandler) {
		...
		if (SimplePreferences.getAccessCount(AdEnviroment.getInstance()
				.getContext(), "enemy") < 16 - diff + 4 * diff) {
			registerUpdateHandler(new TimerHandler(20f, false,
					new ITimerCallback() {
						@Override
						public void onTimePassed(TimerHandler pTimerHandler) {
